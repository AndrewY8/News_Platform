"""
Hybrid Query Analyzer - OpenAI Version
Combines multiple NER/extraction methods using OpenAI instead of Gemini
"""
...
class QueryExtractionModel(BaseModel):
...
class HybridQueryAnalyzer:
    ...
    4. OpenAI (GPT-4) for complex queries (slow, high accuracy)
    """

    def __init__(self, openai_api_key: str):
        """Initialize the hybrid analyzer with OpenAI API key"""
        ...
        logger.info(f"HybridQueryAnalyzer (OpenAI) initialized (spaCy: {SPACY_AVAILABLE}, TextBlob: {TEXTBLOB_AVAILABLE})")
    ...
    def analyze_query(self, query: str) -> QueryIntent:
        ...
        3. If confidence < threshold, use OpenAI fallback
        ...
        # ===== STAGE 2: OpenAI FALLBACK (if needed) =====

        # Use OpenAI if confidence is low or no company found
        if result['confidence'] < 0.7 or not result['companies']:
            logger.info(f"Confidence {result['confidence']:.2f} < 0.7, using OpenAI fallback")
    ...
    def _extract_with_openai(self, query: str) -> Optional[Dict]:
        """Use OpenAI GPT-4 with structured outputs for complex extraction"""
        try:
            ...
            response = self.client.responses.create(
                ...
                input=[{
                    "role": "user",
                    "content": f"""You are a financial query analyzer. Extract structured information from this query.
...
- Intent should describe what the user wants to know (e.g., "earnings_report", "product_news", "market_analysis", "company_strategy")
- Extract all relevant keywords for search
...
- keywords: relevant search terms"""
                        ...
                        "schema": {
                            ...
                            "properties": {
                                "companies": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "List of company names found in the query"
                                },
                                "tickers": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                ...
                                "products": {
                                    "type": "array",
                                    "items": {"type": "string"},
                                    "description": "Specific products mentioned"
                                },
                                "intent": {
                                    "type": "string",
                            ...
                            "required": ["companies", "tickers", "topics", "products", "intent", "keywords"],
            ...
            extraction_model = QueryExtractionModel(**extraction_data)

            logger.debug(f"OpenAI extraction successful: {extraction_model}")

            return {
                'companies': extraction_model.companies,
                'tickers': extraction_model.tickers,
                'topics': extraction_model.topics,
                'products': extraction_model.products,
                'intent': extraction_model.intent,
                'keywords': extraction_model.keywords
            }
    ...
    def _merge_results(self, base: Dict, llm_result: Dict) -> Dict:
        """Merge OpenAI results with base results"""
